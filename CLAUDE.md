...
> **Cursor 试点（2026-07-21 · Vero Path B 探针卡×机台）：** [`docs/HANDOFF_CURSOR_VERO_PROBE_CARD_PILOT.md`](docs/HANDOFF_CURSOR_VERO_PROBE_CARD_PILOT.md) — `AGENT_PROBE_CARD_VERO_PILOT` + `WCHAT_ACCESS_TOKEN`；`simple-agent/invoke` 抽参+解读，表仍服务端直出；关 flag 回 SiliconFlow。  
> **Cursor 交接（2026-07-22 · Agent 全量迁 WChat 最优方案 · 给 Claude Code）：** [`docs/HANDOFF_CURSOR_WCHAT_MIGRATION_OPTIMAL_2026-07-22.md`](docs/HANDOFF_CURSOR_WCHAT_MIGRATION_OPTIMAL_2026-07-22.md) — ①关键字/PRE_LLM 固定工具 + ②未命中则 WChat+工具目录 JSON 选工具；本地 `runTool`；禁看板主链路 MCP；分 Phase 0–5。
> **Vero 驱动通用 Agent 循环内核（2026-07-21 设计/实现 · 2026-07-22 真连验证+合入，子项目 A）：** `runVeroAgentLoop`（`AGENT_VERO_GENERIC_LOOP` + `WCHAT_ACCESS_TOKEN`，默认关，零风险回退）— `PRE_LLM_DIRECT_ROUTES` 原样复用，未命中落 Vero `simple-agent/invoke` JSON action 协议（`tool`/`final`/`chat`）选工具+收尾；不做真流式（`emitTextInChunks` 模拟）。设计：[`docs/superpowers/specs/2026-07-21-vero-generic-agent-loop-design.md`](docs/superpowers/specs/2026-07-21-vero-generic-agent-loop-design.md)；Cursor 真连测试结果：[`docs/HANDOFF_CURSOR_VERO_GENERIC_LOOP_TEST_RESULTS.md`](docs/HANDOFF_CURSOR_VERO_GENERIC_LOOP_TEST_RESULTS.md) — JSON 协议极稳、直连劫持 gate + 超时修复均真连验证生效；⚠️ 最后一轮"必须 final"约束模型不严格遵守（有兜底，非 bug）；字符预算按实测 200k 上下文校准（非 128K），真库大结果集仍待复验。与上一条"WChat 最优方案"（未内置可插拔 backend 抽象，按现有代码风格保留双循环并存）为互补关系，非重复。
