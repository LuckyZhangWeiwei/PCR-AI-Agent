...
> **真库待验：DUT 低良率阈值口径 + JB Star 跨 LOT 多选（2026-07-04 · 待 Cursor）：** [`docs/HANDOFF_CURSOR_REALDB_DUT_YIELD_AND_MULTISELECT_2026-07-04.md`](docs/HANDOFF_CURSOR_REALDB_DUT_YIELD_AND_MULTISELECT_2026-07-04.md) — A：`NF12499.1N` PASS_ID=1 层性质、良品 bin 口径（INF 启发式 vs JB `goodBinIndicesForJbRow`）对比、passId 范围是否窄化；B：`canJoinDutSelectionGroup`（同 Device+LOT 或同 Device+探针卡类型）真实跨 LOT 场景 + DUT×Bin 图核对。
