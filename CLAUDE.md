...
- **`reports/`** — `YieldMonitorReport`, `InfcontrolReport` (both use **`DraggableReportSections`** + nested **`DraggableReportBlocks`** for KPI/chart grids, **`.query-panel`**, layout reset); `AiAgentReport` (**Settings** 可配 **`maxRounds`**，timeout 错误可 **↻ 重试**；工具 JSON 返回后总结依赖 API **`agentLoop` 总结轮 `tool_choice: "none"`**，见 **`pcr-ai-api/CLAUDE.md` §11 条目 11**), `TableRowsReport`, `OverviewReport` (settings only when `embedded`).
