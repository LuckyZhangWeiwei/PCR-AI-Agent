...
- **`reports/`** — `YieldMonitorReport`, `InfcontrolReport` (both use **`DraggableReportSections`** + nested **`DraggableReportBlocks`** for KPI/chart grids, **`.query-panel`**, layout reset); `AiAgentReport` (**Settings** 可配 **`maxRounds`**，timeout 错误可 **↻ 重试**；**New Chat** 会 abort 并重置 loading，见 **`pcr-ai-report/CLAUDE.md` §16**；工具 JSON 返回后总结依赖 API **`agentLoop` 总结轮 `tool_choice: "none"`**，见 **`pcr-ai-api/CLAUDE.md` §11 条目 11**；idle 超时默认 **150s**，见 §11 条目 13), `TableRowsReport`, `OverviewReport` (settings only when `embedded`).
