...
- **`reports/`** — `YieldMonitorReport`, `InfcontrolReport` (both use **`DraggableReportSections`** + nested **`DraggableReportBlocks`** for KPI/chart grids, **`.query-panel`**, layout reset); `AiAgentReport` (**Settings** 可配 **`maxRounds`**、**`streamTimeoutSec`** / **`clientTimeoutSec`**，timeout 错误可 **↻ 重试**；**New Chat** 见 **`pcr-ai-report/CLAUDE.md` §16**；流式泄漏过滤（think / DSML）见 **§18** / **`pcr-ai-api/CLAUDE.md` §11 条目 14**；工具后总结见 **§11 条目 11**), `TableRowsReport`, `OverviewReport` (settings only when `embedded`).
