...
> **被测代码：** 已合并至 `main`，commit 范围 `80b6475..31956f1`（10 个提交：8 个功能实现 + 2 个 2026-07-11 真实模型联调后的路由/输出修复，见 §5/§9）
> **背景：** `aggregate_probe_card_tester_performance` 本身是全新 Agent 工具，本地开发全程在 `INFCONTROL_LAYER_BINS_DUMMY=true` 下用 `docs/JBStart.xlsx` 样本数据验证，**从未跑过真实 Oracle 路径**。合并后又用真实 SiliconFlow MiniMax-M2.5 模型做了一轮端到端联调（仍是 Dummy 数据），发现并修复了两个真实 bug（见 §5/§9）——这两处修复同样只在 Dummy 数据上验证过，**真库 + 真实生产模型的组合还没人测过**。Claude Code 沙箱没有真库访问权限，以下问题必须由 Cursor 用真实数据回答。
> **前置阅读：**
...
| Q5 | Agent 对话里用自然语言问（不是直接调工具）能否正确路由到这个新工具，包括之前被抢答过的那句话？ | 路由抢答 bug 只在真实模型对话里才会暴露，且只在 Dummy 数据上验证过修复，真库 + 生产模型组合未测 |
| Q6 | 中途换卡/中断（TEST INTERRUPT）的 wafer 在真实数据里良率是否始终非负、没有报错？ | 本次修复了「坏 die 超过 GROSSDIE 时良率可能算出负数」的边界（见 §6 修复记录），需要真实中断数据验证 |
| Q7 | 总结轮回复是否真的原样贴出服务端表格（而不是被 LLM 转述成大白话），大数据量（很多卡/很多 lot）时是否仍然如此？ | 2026-07-11 修复只在 Dummy 的 5 行小样本上验证过；真实数据卡数/lot 数多得多，工具 JSON 可能被 `toolResultMaxChars` 截断导致这个修复静默失效（见 §9） |
...
git log --oneline -12   # 顶部应含 31956f1(emit tables verbatim) / 758c282(routing fix) 及以上 8 个 probe-card-perf 提交
cd pcr-ai-api && npm ci && npm run build && npm test
```

`npm test` 期望：**586 个测试，580 pass / 2 fail（已知本地预置问题，见下）/ 4 skip**。2 个失败固定是 `test/jbRouteResolver.test.ts` 里「开关关 → 不调分类器」相关的 2 个用例，根因是 git 追踪的 `pcr-ai-api/runtime-config.json` 里 `jbDeterministicDispatch`/`jbLlmIntentClassifier` 已设 `true`（见 `docs/HANDOFF_CURSOR_JB_CARD_LISTING_SCOPE_2026-07-10.md` §4），与本次改动无关。**如果失败数/失败用例不是这两个，先停下来报告，不要继续验证。**
...
## 5. Q5 — Agent 对话自然语言路由（含一处已修复的抢答 bug，需真库复验）

**背景（2026-07-11 发现并修复，commit `758c282`）：** 用真实 SiliconFlow MiniMax-M2.5 模型 + Dummy 数据测试时发现，句子里同时出现「探针卡」和「最好/最差」会命中一个更早的既有功能（`isCardYieldCompareQuestion`，服务于「哪张卡良率更差」这类单 lot 两卡对比问题）的正则，导致服务端在 LLM 还没机会选工具之前，就直接把问题改发成旧的 `query_jb_bins`，新工具完全拿不到被调用的机会——即使 `aggregate_probe_card_tester_performance` 已经正确注册、prompt 里也写了触发规则。已加一条窄范围的排除规则（出现「组合」，或「机台」与「卡/探针」同时出现时不再算作 card_yield_compare），并在 `test/eval/scenarios/routing-golden.ts` + `test/agentJbDeterministicReply.test.ts` 里补了正则回归测试。**这个修复只跑过 `detectJbReplyMode` 的纯函数单测，从没在真实 Agent 对话 + 真库数据的组合下验证过**，所以下面 4 句里第 1 句是复验这个 bug 修复本身，不是新增需求。
...
1. **（复验路由修复）**「`<真实device>` 这个 device 下最好的探针卡+机台组合是什么，哪张探针卡表现最差」——这句话在修复前 100% 被抢答成 `query_jb_bins`，改后应该正确命中新工具
2. 「哪张探针卡表现最差」（在已经问过上一句、上下文里已经有 device 的情况下追问）
3. 「这张卡良率是不是在变差」（针对上面返回的某张具体卡追问）
4. 「这张卡常见坏 bin 是什么，是不是接触不良」
5. **（回归：不能误伤既有功能）**「哪张卡良率最低」「探针卡哪个最差」——这两句不含「组合」/「机台」，应该仍然命中原来的单 lot 两卡对比逻辑（`query_jb_bins`），**不应该**被本次修复带偏到新工具

**期望：**
- 第 1-4 句命中新工具 `aggregate_probe_card_tester_performance`，不是被路由到 `query_jb_bins`（旧工具）或落空到通用问答。
- 第 5 句（两句）**仍然**命中旧的单 lot 卡对比路径（`query_jb_bins`），确认修复没有误伤既有功能。
...
## 7. Q7 — 总结轮回复是否原样贴表（2026-07-11 修复，commit `31956f1`）

**背景：** 最初 prompt 里只是文字要求"必须原样贴出 `comboRankingMarkdown`/`cardRankingMarkdown`，禁止改写"，实测发现 MiniMax-M2.5 并不遵守——它把表格数字转述成了自己的大白话总结，而且转述时把 pass3 说成了"pass2"（良率数字本身没错，但转述本身就是风险）。修复方式是仿照 `query_jb_bins` 已有的「确定性表直出」架构：新增 `tryRunDeterministicProbeCardPerfSummary`（`agentLoop.ts`），总结轮如果上一个工具是 `aggregate_probe_card_tester_performance`，直接把它返回的四张 markdown 表（按 passId 分组）通过 SSE 原样吐给前端，再单独起一轮「仅写解读/建议」的 LLM 调用（复用既有 `BRIEF_COMMENTARY_SYSTEM`）。这样表格数字完全由服务端保证，不再依赖模型是否"听话"。

**这个修复目前只在 Dummy 的 5 行小样本（单 device、每卡仅 1 lot）上验证过**，有一个已知的潜在风险点需要用真实大数据量核实：

- `tryRunDeterministicProbeCardPerfSummary` 是从上一个工具消息的 `content` 字段 `JSON.parse` 出 payload 的；如果 `aggregate_probe_card_tester_performance` 的返回 JSON 因为卡数/lot 数很多而超过 `toolResultMaxChars`（Settings 默认 20000 字符）被截断（截断后末尾会带"…[数据已截断…]"，不是合法 JSON），`JSON.parse` 会失败，函数会**静默 return false**，退回到总结轮的普通 LLM 路径——也就是说这个修复对大数据量场景可能悄悄失效，回到之前"LLM 转述"的老行为，但不会报错或崩溃。

用一个卡数多、lot 数多的真实 `device` 测试：

```
「<真实device，选一个卡数/lot 数都比较多的> 这个 device 下最好的探针卡+机台组合是什么」
```

**期望：**
- 回复里出现明确的 `## 实测数据` 标题 + 若干个 markdown 表格（`| 排名 | CardId | ... |` 这种竖线表格），表格内容与直接调用工具拿到的 JSON 数字逐一核对一致（不是文字转述）。
- 表格之后出现 `## 分析结论`，其下有 `### 数据解读` 和 `### 专业建议` 两个小节，是纯文字，不应该再包含表格。
- 如果这个 device 数据量确实很大，检查回复是否**没有**出现 `## 实测数据` 表格（即退化成了转述）——如果发生，说明 `toolResultMaxChars` 截断导致了这个修复失效，记录下具体 device 名字、大致的 `groups[].comboRanking`/`cardRanking` 总行数，交回 Claude Code（可能需要提高 `toolResultMaxChars` 上限，或改成从内存缓存而非截断后的 history 字符串里取 payload，参照 `query_jb_bins` 的 `agentJbSessionCache.ts` 缓存机制）。
...
Q5 Agent 自然语言路由（含路由抢答 bug 复验）
  第1句（复验修复）：是否命中新工具=<是/否>
  第2-4句是否命中新工具=<是/是/是 或具体哪句没命中>
  第5句（回归，两句）：是否仍命中旧的 query_jb_bins（不应被带偏）=<是/否>
...
Q7 总结轮是否原样贴表
  大数据量 device=<...>：回复是否出现「## 实测数据」表格=<是/否>
  表格数字与工具原始 JSON 核对=<一致/不一致>
  「## 分析结论」下是否有「### 数据解读」「### 专业建议」两节且不含表格=<是/否>
  若未出现表格（怀疑 toolResultMaxChars 截断）：groups[].comboRanking/cardRanking 总行数估算=<...>
...
- **本次任务是验证，不是改代码。** 如果发现真实 bug（数字不一致、行数保护没触发、路由失败、月度分桶时区问题、大数据量下贴表修复失效等），把具体复现步骤 + 数据带回来，交给 Claude Code 定位修复，不要自行改动 `probeCardTesterPerformance.ts` / `agentToolHandlers.ts` / `agentToolSchemas.ts` / `agentPrompt.ts` / `agentLoop.ts` / `agentJbDeterministicReply.ts`（后两个是 2026-07-11 两处修复touch 到的文件，改动前务必先读懂 `tryRunDeterministicProbeCardPerfSummary` 与 `isCardComboRankingQuestion` 的现有逻辑，避免绕开或重复造轮子）。
