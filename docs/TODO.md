...
- [ ] **【待 FLIP】`JB_DETERMINISTIC_DISPATCH=true`** — 在 Cursor 真库环境运行 `AGENT_EVAL_LIVE=1`，确认实测误分类率 ≤2%，再单独 commit 将 `.env` / `.env.example` 中 `JB_DETERMINISTIC_DISPATCH` 改为 `true`。**Claude 不翻此开关，由用户完成。**
