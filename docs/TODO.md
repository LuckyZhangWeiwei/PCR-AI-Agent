...
- [ ] **【待 Cursor】Agent 回答准确性 8 项修复** — 探针卡四表无标题/无 pass 分组、JB vs DUT 表良率同屏矛盾（`goodBinsByPassId` 未打通）、good bin≠BIN1 全 0 大表、「good bin 是多少」被概况表劫持、lot 列表误挂单 lot DUT 表、listing scope 误带机台致结果不一致、解读跨温度层对比/写出 pass2、cardId count=0 回归。任务书：[`HANDOFF_CURSOR_AGENT_ACCURACY_2026-07-11.md`](HANDOFF_CURSOR_AGENT_ACCURACY_2026-07-11.md) — 2026-07-11 交接
...
- ✅ **阶段三 BUILD：决策驱动确定性预 LLM 派发（dark-launch）** — `agentSemanticDispatchTable.ts`（三入表 mode 纯函数）+ `tryRunSemanticDispatchDirectRoute`（追加 `PRE_LLM_DIRECT_ROUTES`）；黄金集 57→85 条（≥80 ✓）；CI 闸门：`scoreDispatchOnGolden`（派发≥2 + core bug mqygf9mq 必中）；live 闸门：`AGENT_EVAL_LIVE=1` 误分类率≤2%（真库待测）；`JB_DETERMINISTIC_DISPATCH` 默认 **OFF**；429 pass/0 fail/4 skip — 2026-06-29 完成
- [ ] **【待 FLIP】`JB_DETERMINISTIC_DISPATCH=true`** — FLIP 前在 Cursor 真库环境完成**两项**验证(缺一不可)：① 运行 `AGENT_EVAL_LIVE=1`，确认实测误分类率 ≤2%(注:该率分母为黄金集全 85 条、含非派发 mode，是较宽口径)；② **人工 turn1 冒烟**——flag 开后真库提问「n55z 哪个卡测出bin35 多」，确认**首轮**直出 BIN×探针卡 表(而非单 lot 表)，因 flag-ON 服务端派发路径无 CI 覆盖(`runTool` 需 Oracle)，只能真库手测。两项过后再单独 commit 将 `.env` / `.env.example` 中 `JB_DETERMINISTIC_DISPATCH` 改为 `true`。**Claude 不翻此开关，由用户完成。**
- [ ] **【后续】`card_dut_question` / INF 多步确定性派发** — `card_dut_question`（DUT 坏 die 汇总模式）和 INF 晶圆图路由的确定性预派发；另立 spec，阶段三稳定后再推进。
- [ ] **【后续】单 lot 确定性派发** — 针对已知 lot 的 lot_overview / per_slot_bin_ranking 等 single-lot 模式的预派发路由；另立 spec。
