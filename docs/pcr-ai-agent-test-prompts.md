...
### 10f-2. DUT × BIN 关系图（无颜色）

**inf_draw_dut_bin_map — 用图案区分 DUT 和 BIN 的四种关系**

> **通俗理解：**
> 这张图不用颜色，改用**图案**来回答"BIN37 失效是 DUT2 造成的，还是所有 DUT 都有份？"
> - **白色实心** = DUT2 测的 die 且 bin=37（双匹配，最重要）
> - **横线纹理** = DUT2 测的 die 但 bin≠37（该 DUT 的其他失效/良品）
> - **竖线纹理** = bin=37 的 die 但由其他 DUT 测（目标 bin 但别的 DUT 也有）
> - **极暗背景** = 其他 die（不相关）
>
> 如果白色实心 die 远多于竖线 die，说明 BIN37 主要是 DUT2 的问题。

```
WA03P02G NF12551.1N slot 1，画出 DUT2 和 BIN37 的关系图，看看 BIN37 失效是否主要由 DUT2 造成。
```

**聚焦高温 pass：**

```
WA03P02G NF12551.1N lot slot 1，pass3（高温）下，DUT3 和 BIN55 的关系如何？BIN55 失效主要集中在 DUT3 还是均匀分布在各 DUT？画关系图。
```

**与 DUT 良率统计联动：**

```
WA03P02G NF12551.1N lot slot 1：先查各 DUT 良率（inf_site_stats），找出良率最低的 DUT，再查失效最多的坏 bin（inf_yield_loss_breakdown），最后画出这个 DUT 和这个坏 bin 的关系图，判断是否为系统性问题。
```
