...
| **中断次数** (`testInterruptCount`) | 同 (slot,passId)：**INTERRUPT 行数**；无 INTERRUPT 行时 **max(PASSNUM)−min(PASSNUM)**；同 PASSNUM 多行 TEST 时为 **TEST 行数−1**。**禁止**把良率表段数当成次数。见 `testInterruptCountMarkdown`。 |
| **逐段良率** (`interruptSegments`) | 多次 INTERRUPT：**中断1…N** → 续测完成 → 整片合并；见 `buildYieldInterruptSegments`。 |
