...
## 重构核心架构原则（2026-05-24 补充）

### 主线：探针卡健康调查漏斗

当前两张报表**按数据源组织**（YM 表 vs JB 表），用户需要在脑子里把两张图拼起来。重构目标是改成**按调查流程组织**。

**自然的调查主线：**
```
有什么报警？→ 哪张探针卡？→ 哪个 DUT 有问题？→ 影响了哪些 lot/wafer？→ 具体失效的是哪个 Bin？
```

**YM 和 JB 在主线上的分工：**
- **YM（发现问题）** = 「哪张卡有 DUT 失衡报警，需要关注」
- **JB（定位问题）** = 「这张卡测出的 wafer 到底哪些 Bin 失效了」

**探针卡是两张报表的锚点**：选中一张卡后，YM 和 JB 的数据都围绕它展开。

### 每张报表内部的漏斗结构

**YM 报表（发现问题）：**
```
① 探针卡报警排名总览（第一眼知道哪张卡最需要关注）
      ↓ 点击某张卡
② 该卡的 DUT 失衡趋势（时间轴 + 报警次数）
      ↓
③ 关联 lot 列表 → 跳转 JB 查 Bin 详情
```

**JB 报表（定位问题）：**
```
① 坏 Bin 全局分布（第一眼知道哪个 Bin 失效最多）
      ↓ 点击某个 Bin
② 关联 lot 列表（该 Bin 失效最多的 lot，带"主要坏 Bin"列）
      ↓
③ Lot 内 DUT×Bin 聚合（哪个 DUT site 贡献了这个 Bin）
      ↓
④ 跳回 YM → 查这张卡的历史报警
```
...
1. **漏斗结构**：每张报表从宏观→微观，每一层点击驱动下一层，不允许"平铺所有维度"的布局
2. **探针卡是锚点**：YM 选中一张卡，JB 自动带入该卡的 lot 范围（或提供快捷跳转），反之亦然
3. **可下钻 = 有故事**：能下钻的 chart 一定引导到"下一层要看什么"，不能下钻的只是总览数字；用视觉明确区分（hover 红色禁止圈 vs 正常指针）
4. **精简重复**：YM/JB 相同维度分析（如 by-lot 趋势）提取为共用组件，不在两处各自维护
