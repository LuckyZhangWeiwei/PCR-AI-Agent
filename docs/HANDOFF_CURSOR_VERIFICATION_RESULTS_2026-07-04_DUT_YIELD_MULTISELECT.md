...
| 优先级 | 动作 | 文件 hint |
|---|---|---|
| **P0** | 取数良品 bin：JB PASSBIN 优先 | `lotUnderperformingDuts.ts`、`lotUnderperformingDutsResolve.ts` + Dummy |
| **P1** | Agent 路由：DUT 75% 阈值 → `query_lot_underperforming_duts` | `agentPrompt.ts`、`agentToolHandlers.ts`、路由检测 |
