...
第 2 轮请求里我们带了 `tools + tool_choice: "auto"`，相当于对模型说"你自由选择，可以调工具也可以回答"。DeepSeek 觉得"再查一次更好"，于是又调了工具，进入死循环，直到超时。
...
总结轮（工具结果已经有了）不再带 `tools`，模型没有工具可调，只能输出文字。同时在 system 里加一句提示："工具已完成，请立即给中文结论"。
