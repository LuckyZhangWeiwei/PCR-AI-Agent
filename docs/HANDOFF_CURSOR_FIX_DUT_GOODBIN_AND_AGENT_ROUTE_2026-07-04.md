...
| **P1** Agent 直连 `query_lot_underperforming_duts` | ✅ 已修 | 放宽 `isLotUnderperformingDutQuestion`，NF12499 类问句命中 A 路 |
| **Part B** JB Star 跨 LOT 多选 | ⏭ 未动 | 真库 API 无 bug，无需回改 `infDutSelection.ts` |
| **部署后复验** | ⏭ 待做 | `npm run build && pm2 reload` 后 curl + Agent 各测一次 |
...
| 优先级 | 项 | 说明 |
|---|---|---|
| 低 | LLM 路径仍误选工具 | A 路未命中时的 prompt / `detectPendingQuery` 可再加强；当前 NF12499 类问句应走 A 路 |
