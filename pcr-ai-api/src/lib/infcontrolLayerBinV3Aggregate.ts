...
export const INFCONTROL_V3_AGGREGATE_DOCUMENTATION =
  ...
  "聚合统计的是筛选下的全量匹配行。**SUM 仅累计坏 bin**：PASSBIN 按「-」拆成的整段下标（0…255）视为 good bin（与列表 bins[].isGoodBin、与 /infcontrol-layer-bins/v2/top-bad-bins 的 token 规则一致），这些列 die 不计入；**不再**使用 v1 聚合的「BIN1 恒排除 + 仅 N-M 两端排除」。**groupBy** 除原有 token 外可增加 **probeCardType**（与列表 **PROBECARDTYPE** 一致：CARDID 首个「-」前段）。" +
