...
import { buildYieldMonitorTriggerMatchingCountSql } from "../yieldMonitorTriggerAggregate.js";
import {
  buildYieldMonitorHostnameSummarySql,
  buildYieldMonitorProbeCardSummarySql,
  buildYieldMonitorTriggerTopSql,
} from "../../yieldMonitorTriggerSql.js";
import {
...
import { YIELD_MONITOR_V4_AGGREGATE_DOCUMENTATION } from "../../apiV4Docs.js";
...
} from "../periodAlarmTrend/periodAlarmTrendTypes.js";
import { parsePeriodAlarmTrendQuery } from "../periodAlarmTrend/periodAlarmTrendParse.js";
import {
...
} from "../periodAlarmTrend/periodAlarmTrendSql.js";
import {
...
} from "../periodAlarmTrend/periodAlarmTrendAggregate.js";
 ...
 */
export async function handleYieldMonitorTriggers(req: Request, res: Response) {
 ...
 */
export async function handleYieldMonitorTriggersV3(req: Request, res: Response) {
 ...
 */
export async function handleYieldMonitorTriggersV3Aggregate(
  req: Request,
  res: Response
) {
 ...
 */
export async function handleYieldMonitorTriggersV3Combined(
  req: Request,
  res: Response
) {
  const parsed = parseYieldMonitorTriggerV3Query(
...
/**
 * Pure query-plan builder for the period-alarm-trend Oracle path: turns the
 * parsed query into the 5 SQL statements + 3 bind sets the handler executes.
 * Extracted so the handler body reads as parse → dummy branch → build plan →
 * execute → merge → respond, without inlining 8 near-identical builder calls.
 */
function buildPeriodAlarmTrendQueryPlan(parsed: ParsePeriodAlarmTrendOk) {
  return {
    sql: buildPeriodAlarmTrendSql(parsed.activityWhereSql, parsed.buckets.length),
    topTestersSql: buildPeriodAlarmTrendTopTestersSql(
      parsed.activityWhereSql,
      parsed.buckets.length
    ),
    topDevicesSql: buildPeriodAlarmTrendTopDevicesSql(
      parsed.activityWhereSql,
      parsed.buckets.length
    ),
    topProbeCardsSql: buildPeriodAlarmTrendTopProbeCardsSql(
      parsed.activityWhereSql,
      parsed.buckets.length
    ),
    jbSlotSql: buildPeriodAlarmJbSlotTuplesSql(
      parsed.jbSlotWhereAndSql,
      parsed.buckets.length
    ),
    mainBinds: periodAlarmTrendMainBinds(parsed),
    topBinds: periodAlarmTrendTopBinds(parsed),
    jbSlotBinds: periodAlarmTrendJbSlotBinds(parsed),
  };
}
 ...
 */
export async function handleYieldMonitorTriggersV3PeriodAlarmTrend(
  req: Request,
  res: Response
) {
  ...
  const {
    ...
    topBinds,
    jbSlotBinds,
  } = buildPeriodAlarmTrendQueryPlan(parsed);
  try {
 ...
 */
export async function handleYieldMonitorTriggersV4(req: Request, res: Response) {
  const parsed = parseYieldMonitorTriggerV3Query(
    req.query as Record<string, unknown>
  );
  if (!parsed.ok) {
 ...
 */
export async function handleYieldMonitorTriggersV4Aggregate(
  req: Request,
  res: Response
) {
  const parsed = parseYieldMonitorTriggerV3AggregateQuery(
