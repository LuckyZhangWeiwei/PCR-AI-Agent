// pcr-ai-api/src/lib/agent/prompt/agentPrompt.ts
//
...
//              is preserved; classifyIntent() (see ./agentPromptIntent.ts) controls
//              which sections are injected per-turn based on the user's inferred intent.
//
// ╔══════════════════════════════════════════════════════════════════════════╗
...
// ║    └ 内含 11 个 ### 子节；定义见 ./sections/domainSection.ts              ║
...
import type { PromptIntent } from "./agentPromptIntent.js";
import { SEC_DOMAIN } from "./sections/domainSection.js";
...
| **DUT×BIN 数量汇总**（哪个DUT坏bin最多/各DUT数量/**某BIN集中在哪些DUT**/**「是否聚集到某个DUT」「BINxx是否集中在某DUT」**，含承接前轮卡级结论的追问）| \`query_lot_dut_bin_agg(device, focusBin=<上一轮的BIN>)\`（整批）/ \`query_inf_site_bin_by_dut\`（单片）；**指定 DUT 问哪个坏 bin** → \`query_inf_site_bin_by_dut(..., focusDut)\` 读 \`focusDutBins\`；**禁止**用 \`aggregate_jb_bins\` 代答 | 跳过第一级直接调 \`inf_site_stats\` 或 \`inf_draw_dut_bin_map\`；用 lot/device 坏 BIN 排行表当「某 DUT 的坏 bin」结论；从 Top8 DUT 列表「看不到某 DUT」推断零失效 |
