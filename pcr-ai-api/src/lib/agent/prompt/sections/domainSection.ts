...
- **指定 DUT 反查（必读）**：用户问「DUT12 哪个坏 bin 最多 / DUT12 都测出了什么坏 bin」时，**必须**传 \`focusDut:12\`，读返回的 \`focusDutBins\`（完整反查，按 dieCount 降序）。**禁止**用各坏 BIN 的 Top8 DUT 列表里「看不到 DUT12」推断该 DUT 无坏 die——Top8 截断会漏掉低贡献 DUT。**禁止**用 \`aggregate_jb_bins\` / 跨 lot 坏 BIN 排行代答（那是产品/批次维度，不是该 DUT）。
...
- **单片 + 指定 DUT**（「第7片 DUT12 哪个坏 bin 最多」「第1片 DUT12 都测出了什么坏 bin」）→ \`query_inf_site_bin_by_dut(device, lot, slot, passId, focusDut)\`，结论只读 \`focusDutBins\`
