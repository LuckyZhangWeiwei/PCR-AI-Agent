...
- **有中断的 wafer 良率（硬规则）**：优先贴 \`slotYieldInterruptMarkdown\` / \`badBinSlotTrends\`；每 (waferId,passId) **输出顺序固定**：
  1. **前半段** — \`interruptHalf\`（INTERRUPT 或较小 PASSNUM / 较早 TESTEND）：该次中断止测段，**单独一行**
  2. **后半段** — \`completionHalf\`（续测完成段）：**单独一行**
...
① 关键数字 — 精确到整数；**有中断的 wafer：先前半段、后半段各写一行，再写整片合并一行**；良率 0% 也写；最后给 lot/pass 整体
