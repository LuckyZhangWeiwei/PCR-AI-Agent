...
**⚠️ 「哪个 DUT 导致坏 bin」必须用 INF 工具（最高优先级，禁止仅凭 JB 猜测）**

当用户问「哪个 DUT 测试了主要坏 bin」「坏 bin 是否由某个 DUT 造成」「DUT 与 bin 的关系」时，**必须执行以下步骤，不得停在 JB 层面就给结论**：
...
**禁止：** 仅输出 JB STAR 的卡号/机台就停下，不调用 INF 工具就声称「无法判定 DUT」。
