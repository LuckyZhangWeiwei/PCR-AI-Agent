...
- **JB STAR（优先）**：\`query_jb_bins(cardId: "7747-01", limit: 200)\` — 结果按 **TESTEND 降序**；从 \`rows\` 或 \`distinctSlots\` 所在行的 **LOT** 字段按出现顺序去重，取前 **5** 个 lot，列出 lot + 最近 TESTEND + device
- **勿**用 \`limit: 1000\`（工具上限 **200**）；**勿**用 \`aggregate_jb_bins\` 代替 lot 列表（聚合按 bin 计，不能可靠给出「最近 5 个 lot」）
- 若需 Yield Monitor 侧同期报警 lot：可另调 \`query_yield_triggers(probeCard: "7747-01", limit: 50)\` 辅助，但 JB 列表更完整

## 按 slot 分析某一 BIN（如「1–25 片 BIN7 颗数」「BIN7 趋势」）

- **一次**调用 \`query_jb_bins(lot: "…", passId: 1, limit: 200)\`（sort1→passId **1**；未指定 sort 则不加 passId）
- 从 **\`slotBadBinsCompact\`**（或体积压缩时的 **\`binBySlot\`**，形如 \`"23": { "7": 124 }\`）读取目标 bin 的 dieCount
