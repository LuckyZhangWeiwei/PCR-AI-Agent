...
- 单 lot 各片良率 → 无中断用 \`slotYieldPivotMarkdown\`；**有测试中断**时必须先贴 \`slotYieldInterruptMarkdown\`（每 (slot,passId) 三行：**整片正片 → 前半段 → 后半段**，0% 也写），再列无中断片；或读 \`slotYieldSummary\`（**每个 slot×passId 一条**，25 片×2 sort=50 条，**禁止**压成 25 行）。批次总良率 → \`yieldByPassId\` **按 pass 分开**，禁止把 pass3+pass5 的 die 相加。
- 未指定 sort/pass 时 **不加** \`passId\` 过滤；结论可注明 passId（1=常温/sort1，3=高温/sort2，5=低温/sort3）。
...
- **有哪些 sort**：以 \`passIdsPresent\` 为准；**禁止**写「无 sort1」除非 passIdsPresent 不含 1
...
- **有中断 slot**：\`slotYieldInterruptMarkdown\`（每 (slot,passId) **整片正片→前半→后半**，0% 也写）
...
- **slot 良率**：优先直接贴工具返回的 \`slotYieldInterruptMarkdown\`；或读 \`slotYieldSummary\` — \`hasInterrupt:true\` 时每 (slot,passId) **必须分别汇报三项**，且**输出顺序固定**（按测试时间先后，勿打乱）：
...
- **一次** \`query_jb_bins(lot: "…", limit: 200)\`（lot 全量行；勿只查 passId=3 后推断常温）
- **首选** \`badBinSlotTrends\`：含 **BIN 合计/前半/后半** 与 **良率 整片/前半/后半**；有中断片另附三行明细表
- 其次 \`slotsByPassId\`：若 passId=1 的 slots 含 slot 1，**禁止**写「slot 1 无常温/无 sort1 数据」；有 \`hasInterrupt\` 须注明中断并仍列出 BIN 颗数
- 备用 \`slotBadBinsCompact\` / \`binBySlot\`（键 \`slot:passId:cardId\`）；多卡同层须分键后相加
- 必须覆盖 **\`distinctSlots\` 全部 slot**（通常 1–25），升序；无该层测试行才可写「—」
- **禁止**用 sort2 良率代替 sort1 结论（例如 slot 1 仅有 passId=3 时不能当作「无常温」）
