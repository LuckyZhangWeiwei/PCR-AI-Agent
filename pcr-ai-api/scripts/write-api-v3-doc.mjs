...
const filters = await import(
  pathToFileURL(join(root, "../dist/lib/yieldMonitorTriggerFilters.js")).href
);
const { buildInfcontrolLayerBinsV3Sql, buildYieldMonitorTriggersV3Sql } =
  apiV3;

const ic = buildInfcontrolLayerBinsV3Sql("");
const yParsed = filters.parseYieldMonitorTriggerV3Query({});
if (!yParsed.ok) throw new Error(yParsed.error);
const y = buildYieldMonitorTriggersV3Sql(yParsed.whereSql);
const icEx = buildInfcontrolLayerBinsV3Sql(
  "UPPER(TRIM(t1.DEVICE)) = UPPER(:ic3_device) AND t2.TESTEND >= :ic3_testend_lo"
);
const yParsedEx = filters.parseYieldMonitorTriggerV3Query({
  device: "WA03P02G",
  timeStampBegin: "2026-05-13T00:00:00.000Z",
});
if (!yParsedEx.ok) throw new Error(yParsedEx.error);
const yEx = buildYieldMonitorTriggersV3Sql(yParsedEx.whereSql);
