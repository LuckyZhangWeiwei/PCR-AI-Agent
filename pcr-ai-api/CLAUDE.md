...
| **INF wafer pass × bin × probe DUT** | **`GET /api/v1/inf-analysis/site-bin-bylot`**（亦挂载 `/api/v3`、`/api/v4`）：**只读 GET**，**不修改/删除 INF**。单片 **`infPath`+`passId`**；Lot **`device`+`lot`+`passId`**（可选 **`probeCardType`**：无则扫 lot 目录全部 `r_1-{slot}`，有则 JB 同卡型过滤）；Device **`device`+`probeCardType`+`passId`**（无 `lot`）。实现：**`infAnalysisRoutes.ts`** → **`outputSiteBinByLot.ts`** / **`siteBinByLotWaferResolve.ts`**（JB DISTINCT lot+slot）/ **`buildInfPath.ts`** → Perl **`output_site_bin_bylot.pl --json`**（**`PASS_TYPE='TEST'`**）。**用法 curl：** [`docs/SITE_BIN_BY_LOT_API.md`](docs/SITE_BIN_BY_LOT_API.md) §2。Dummy：**`SITE_BIN_BY_LOT_DUMMY`** 或 **`INFCONTROL_LAYER_BINS_DUMMY`**。测试 **`test/outputSiteBinByLot.test.ts`**。集成 **[`../docs/SITE_BIN_BY_LOT_INTEGRATION.md`](../docs/SITE_BIN_BY_LOT_INTEGRATION.md)**。 |
...
7. **INF site-bin-bylot（2026-05-20，聚合 2026-05-25）**：**`GET /api/v1/inf-analysis/site-bin-bylot`** — **只读** INF（Perl `LoadINF` + Node `readdir`/`access(R_OK)`，无写删）。**单片：** `infPath`+`passId`。**Lot 聚合：** `device`+`lot`+`passId`；无 `probeCardType` → 扫 `{INF_STORAGE_ROOT}/{DEVICE}/{LOT}/` 下全部 `r_1-{slot}`；有 `probeCardType` → JB TEST 行过滤同卡型后累加。**Device 聚合：** `device`+`probeCardType`+`passId`（勿传 `lot`）。响应 **`passes[].bins[].bin`** / **`duts[].dut`** / **`dieCount`**；聚合另有 **`waferCount`**、`waferSlots`、`meta.aggregateScope`。上限 **`SITE_BIN_BY_LOT_MAX_WAFERS`**（25）/ **`SITE_BIN_BY_LOT_MAX_WAFERS_DEVICE`**（100）。**完整 curl：** [`docs/SITE_BIN_BY_LOT_API.md`](docs/SITE_BIN_BY_LOT_API.md)。发布 **`npm run build`**（copy perlscripts）。报表仍用单片 **`buildInfPath`**；**`query_inf_site_bin_by_dut`** 未改。
