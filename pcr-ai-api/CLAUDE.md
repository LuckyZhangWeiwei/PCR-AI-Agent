...
7. **INF site-bin-bylot（2026-05-20，聚合 2026-05-25）**：**`GET /api/v1/inf-analysis/site-bin-bylot`** — **只读** INF（Perl `LoadINF` + Node `readdir`/`access(R_OK)`，无写删）。**单片：** `infPath`+`passId`。**Lot 聚合：** `device`+`lot`+`passId`；无 `probeCardType` → 扫 lot 目录全部 `r_1-{slot}`；有 `probeCardType` → JB TEST 同卡型过滤。**Device 聚合：** `device`+`passId`（勿传 `lot`）；`probeCardType` **可选**（省略 → JB 仅一种卡型则自动，多种 → 400）。生产示例：`?device=WA03P02G&passId=1`。**交接：** [`../docs/HANDOFF_SITE_BIN_BY_LOT_AGG.md`](../docs/HANDOFF_SITE_BIN_BY_LOT_AGG.md)。上限 **`SITE_BIN_BY_LOT_MAX_WAFERS`**（25）/ **`SITE_BIN_BY_LOT_MAX_WAFERS_DEVICE`**（100）。**curl：** [`docs/SITE_BIN_BY_LOT_API.md`](docs/SITE_BIN_BY_LOT_API.md)。报表仍用单片 **`buildInfPath`**；**`query_inf_site_bin_by_dut`** 未改。
