...
7. **INF site-bin-bylot（2026-05-20，聚合 2026-05-25）**：**`GET /api/v1/inf-analysis/site-bin-bylot`** — **只读** INF。**Device/Lot+JB：** 未传时间参数时默认 **TESTEND 最近一年**（**`siteBinByLotTestEndWindow.ts`**，与 v3 层控一致）；避免全历史上千片触发 **`SITE_BIN_BY_LOT_MAX_WAFERS_DEVICE`**（100）。**Device：** `device`+`passId`；`probeCardType` 可选。**交接：** [`../docs/HANDOFF_SITE_BIN_BY_LOT_AGG.md`](../docs/HANDOFF_SITE_BIN_BY_LOT_AGG.md)。**curl：** [`docs/SITE_BIN_BY_LOT_API.md`](docs/SITE_BIN_BY_LOT_API.md)。
