...
| **INF wafer pass × bin × probe DUT** | **`GET /api/v1/inf-analysis/site-bin-bylot`**（亦挂载 `/api/v3`、`/api/v4`）：**只读 GET**，**不修改/删除 INF**。单片 **`infPath`+`passId`**；Lot **`device`+`lot`+`passId`**（可选 **`probeCardType`**：无则扫 lot 目录全部 `r_1-{slot}`，有则 JB 同卡型过滤）；Device **`device`+`passId`**（无 `lot`；**`probeCardType` 可选** — 省略时 JB 推断唯一卡型，多种则 400）。实现：**`infAnalysisRoutes.ts`** → **`outputSiteBinByLot.ts`** / **`siteBinByLotWaferResolve.ts`** / **`buildInfPath.ts`** → Perl **`output_site_bin_bylot.pl --json`**（**`PASS_TYPE='TEST'`**）。**curl：** [`docs/SITE_BIN_BY_LOT_API.md`](docs/SITE_BIN_BY_LOT_API.md) §2。**聚合交接：** [`../docs/HANDOFF_SITE_BIN_BY_LOT_AGG.md`](../docs/HANDOFF_SITE_BIN_BY_LOT_AGG.md)。Dummy：**`SITE_BIN_BY_LOT_DUMMY`** 或 **`INFCONTROL_LAYER_BINS_DUMMY`**。测试 **`test/outputSiteBinByLot.test.ts`**。wafer 产品/Agent：**[`../docs/SITE_BIN_BY_LOT_INTEGRATION.md`](../docs/SITE_BIN_BY_LOT_INTEGRATION.md)**。 |
