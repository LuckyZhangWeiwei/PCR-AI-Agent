...
23. **Agent device/NXP 数据脱敏（2026-07-05）**：
   ...
   - **审计证据（2026-07-17）**：开关打开时每次 `streamSiliconFlow` 写带 **ISO 时间戳** 的 JSONL + `console.info`（`[agentDataMasking/audit]`）。字段含 `ts` / `event`（`outbound_mask` | `inbound_unmask`）/ `dictSize` / `dictBuiltAt` / `deviceReplacements` / `nxpReplacements`（或 inbound 的 `*TokensRestored`）等；**从不记录真实 device / 原文**。默认文件 **`logs/agent-data-masking-audit.jsonl`**（已 gitignore）；`AGENT_DATA_MASKING_AUDIT_PATH` 可改路径；`AGENT_DATA_MASKING_AUDIT=false` 只关文件、仍打控制台。实现：**`agentDataMaskingAudit.ts`** + `maskWithStats` / `unmaskWithStats`。
