...
| [`docs/API_V3.md`](docs/API_V3.md) | **v3 列表** SQL（`apiV3ListSql.ts`）；**v3 聚合** SQL 见 `yieldMonitorTriggerV3Aggregate.ts`、`infcontrolLayerBinV3Aggregate.ts`；层控 **`…/v3/aggregate`** 仅累计**坏 bin** die（`buildInfcontrolLayerBinAggregateSql` **`v3-hyphen-tokens`** + dummy **`forEachBadBinDieContribution`**，见 **AI_AGENT_API.md §7.6**） |
