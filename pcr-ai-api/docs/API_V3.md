# API v3（层控 / 产量监控）

本文档记录 **v3** 路由的**原始 SQL 模板**（与当前编译产物 `dist/lib/apiV3ListSql.js` 一致）。**HTTP 前缀**推荐 **/api/v3**（与 **/api/v1** 同路由器；manifest 行为见 [**AI_AGENT_API.md**](./AI_AGENT_API.md) 文首表、**§6.2**、**§8.3**）。业务说明、通俗理解见主文档 **§7**；**可复制 URL 与 curl** 见 **§8**；筛选参数表亦见 **§8.4**、**§8.6**。

**v3 聚合**（`/infcontrol-layer-bins/v3/aggregate`、`/yield-monitor-triggers/v3/aggregate`）的 SQL 不在此文件展开，见源码 **`src/lib/yieldMonitorTriggerV3Aggregate.ts`**、**`src/lib/infcontrolLayerBinV3Aggregate.ts`**（层控 UNPIVOT 与 **`infcontrolLayerBinAggregate.ts`** 共用 SQL 片段）。

**层控 `…/v3/aggregate`（仅坏 bin die）**：Oracle 路径 **`buildInfcontrolLayerBinAggregateSql(..., "v3-hyphen-tokens")`**；**PASSBIN** 按 **-** 拆出的整段下标为 **good bin**，**SUM** 不累计其 die（与 v3 列表 **`bins[].isGoodBin`**、**/v2/top-bad-bins** 的 token 规则一致）。**INFCONTROL_LAYER_BINS_DUMMY=true** 时 **`infcontrolLayerBinDummy.ts`** 内 **`forEachBadBinDieContribution`** 与 v2 top-bad-bins dummy 共用循环。见 [**AI_AGENT_API.md**](./AI_AGENT_API.md) **§4**、**§7.6**。

字符串筛选由解析器生成 **`UPPER(TRIM(列)) = UPPER(:bind)`**，与仓库样例表 **`docs/JBStart.xlsx`**（层控）、**`docs/delta-diff.xlsx`**（产量）中的大小写习惯一致；查询参数**键名**不区分大小写（含 `limit`）。**产量 v3**（`/yield-monitor-triggers/v3` 与 `/v3/aggregate`）在 **`parseYieldMonitorTriggerV3Query`** 中**恒**追加 **`UPPER(TRIM(t."TYPE")) = UPPER(:v3_type_scope)`**（`:v3_type_scope` = **`delta_diff`**）；Dummy 路径由 **`filterYieldMonitorDummyRowsMatchingV3`** 同步筛选 **`TYPE`**。

## 源码位置

| 项 | 路径 |
| --- | --- |
| SQL 拼接 | `src/lib/apiV3ListSql.ts` |
| 层控 v3 筛选解析 | `src/lib/infcontrolLayerBinFilters.ts` → `parseInfcontrolLayerBinsV3Query` |
| 产量 v3 筛选解析 | `src/lib/yieldMonitorTriggerFilters.ts` → `parseYieldMonitorTriggerV3Query` |
| HTTP 路由 | `src/routes/api.ts` |

## 绑定变量

- **`:lim`**：由查询参数 `limit` 解析（默认 200，上限 500；**键名不区分大小写**，见 `clampLimitFromQuery`），与其它筛选 bind 一并传入 `conn.execute`。

---

## 1. `GET /api/v3/infcontrol-layer-bins/v3`

### 1.1 无额外筛选（仅 `PASSTYPE = 'TEST'`）

```sql
SELECT
ic.DEVICE,
    ic.LOT,
    ic.SLOT,
    ic.MESLOT,
    lb.TESTERID,
    lb.TSTYPE,
    lb.CARDID,
    lb.PIBID,
    lb.PROBE,
    lb.GROSSDIE,
    lb.PASSID,
    lb.PASSNUM,
    lb.TESTSTART,
    lb.TESTEND,
    lb.LAYERNAME,
    lb.PASSRESUME,
    lb.PASSTYPE,
    lb.PASSBIN,
    lb.BIN1,
    lb.BIN2,
    lb.BIN3,
    lb.BIN4,
    lb.BIN5,
    lb.BIN6,
    lb.BIN7,
    lb.BIN8,
    lb.BIN9,
    lb.BIN10,
    lb.BIN11,
    lb.BIN12,
    lb.BIN13,
    lb.BIN14,
    lb.BIN15,
    lb.BIN16,
    lb.BIN17,
    lb.BIN18,
    lb.BIN19,
    lb.BIN20,
    lb.BIN21,
    lb.BIN22,
    lb.BIN23,
    lb.BIN24,
    lb.BIN25,
    lb.BIN26,
    lb.BIN27,
    lb.BIN28,
    lb.BIN29,
    lb.BIN30,
    lb.BIN31,
    lb.BIN32,
    lb.BIN33,
    lb.BIN34,
    lb.BIN35,
    lb.BIN36,
    lb.BIN37,
    lb.BIN38,
    lb.BIN39,
    lb.BIN40,
    lb.BIN41,
    lb.BIN42,
    lb.BIN43,
    lb.BIN44,
    lb.BIN45,
    lb.BIN46,
    lb.BIN47,
    lb.BIN48,
    lb.BIN49,
    lb.BIN50,
    lb.BIN51,
    lb.BIN52,
    lb.BIN53,
    lb.BIN54,
    lb.BIN55,
    lb.BIN56,
    lb.BIN57,
    lb.BIN58,
    lb.BIN59,
    lb.BIN60,
    lb.BIN61,
    lb.BIN62,
    lb.BIN63,
    lb.BIN64,
    lb.BIN65,
    lb.BIN66,
    lb.BIN67,
    lb.BIN68,
    lb.BIN69,
    lb.BIN70,
    lb.BIN71,
    lb.BIN72,
    lb.BIN73,
    lb.BIN74,
    lb.BIN75,
    lb.BIN76,
    lb.BIN77,
    lb.BIN78,
    lb.BIN79,
    lb.BIN80,
    lb.BIN81,
    lb.BIN82,
    lb.BIN83,
    lb.BIN84,
    lb.BIN85,
    lb.BIN86,
    lb.BIN87,
    lb.BIN88,
    lb.BIN89,
    lb.BIN90,
    lb.BIN91,
    lb.BIN92,
    lb.BIN93,
    lb.BIN94,
    lb.BIN95,
    lb.BIN96,
    lb.BIN97,
    lb.BIN98,
    lb.BIN99,
    lb.BIN100,
    lb.BIN101,
    lb.BIN102,
    lb.BIN103,
    lb.BIN104,
    lb.BIN105,
    lb.BIN106,
    lb.BIN107,
    lb.BIN108,
    lb.BIN109,
    lb.BIN110,
    lb.BIN111,
    lb.BIN112,
    lb.BIN113,
    lb.BIN114,
    lb.BIN115,
    lb.BIN116,
    lb.BIN117,
    lb.BIN118,
    lb.BIN119,
    lb.BIN120,
    lb.BIN121,
    lb.BIN122,
    lb.BIN123,
    lb.BIN124,
    lb.BIN125,
    lb.BIN126,
    lb.BIN127,
    lb.BIN128,
    lb.BIN129,
    lb.BIN130,
    lb.BIN131,
    lb.BIN132,
    lb.BIN133,
    lb.BIN134,
    lb.BIN135,
    lb.BIN136,
    lb.BIN137,
    lb.BIN138,
    lb.BIN139,
    lb.BIN140,
    lb.BIN141,
    lb.BIN142,
    lb.BIN143,
    lb.BIN144,
    lb.BIN145,
    lb.BIN146,
    lb.BIN147,
    lb.BIN148,
    lb.BIN149,
    lb.BIN150,
    lb.BIN151,
    lb.BIN152,
    lb.BIN153,
    lb.BIN154,
    lb.BIN155,
    lb.BIN156,
    lb.BIN157,
    lb.BIN158,
    lb.BIN159,
    lb.BIN160,
    lb.BIN161,
    lb.BIN162,
    lb.BIN163,
    lb.BIN164,
    lb.BIN165,
    lb.BIN166,
    lb.BIN167,
    lb.BIN168,
    lb.BIN169,
    lb.BIN170,
    lb.BIN171,
    lb.BIN172,
    lb.BIN173,
    lb.BIN174,
    lb.BIN175,
    lb.BIN176,
    lb.BIN177,
    lb.BIN178,
    lb.BIN179,
    lb.BIN180,
    lb.BIN181,
    lb.BIN182,
    lb.BIN183,
    lb.BIN184,
    lb.BIN185,
    lb.BIN186,
    lb.BIN187,
    lb.BIN188,
    lb.BIN189,
    lb.BIN190,
    lb.BIN191,
    lb.BIN192,
    lb.BIN193,
    lb.BIN194,
    lb.BIN195,
    lb.BIN196,
    lb.BIN197,
    lb.BIN198,
    lb.BIN199,
    lb.BIN200,
    lb.BIN201,
    lb.BIN202,
    lb.BIN203,
    lb.BIN204,
    lb.BIN205,
    lb.BIN206,
    lb.BIN207,
    lb.BIN208,
    lb.BIN209,
    lb.BIN210,
    lb.BIN211,
    lb.BIN212,
    lb.BIN213,
    lb.BIN214,
    lb.BIN215,
    lb.BIN216,
    lb.BIN217,
    lb.BIN218,
    lb.BIN219,
    lb.BIN220,
    lb.BIN221,
    lb.BIN222,
    lb.BIN223,
    lb.BIN224,
    lb.BIN225,
    lb.BIN226,
    lb.BIN227,
    lb.BIN228,
    lb.BIN229,
    lb.BIN230,
    lb.BIN231,
    lb.BIN232,
    lb.BIN233,
    lb.BIN234,
    lb.BIN235,
    lb.BIN236,
    lb.BIN237,
    lb.BIN238,
    lb.BIN239,
    lb.BIN240,
    lb.BIN241,
    lb.BIN242,
    lb.BIN243,
    lb.BIN244,
    lb.BIN245,
    lb.BIN246,
    lb.BIN247,
    lb.BIN248,
    lb.BIN249,
    lb.BIN250,
    lb.BIN251,
    lb.BIN252,
    lb.BIN253,
    lb.BIN254,
    lb.BIN255,
    lb.PASSRESULT
FROM (
    SELECT t2.ROWID AS lb_rid
    FROM INFCONTROL t1
    INNER JOIN INFLAYERBINLIST t2 ON t1.KEYNUMBER = t2.KEYNUMBER
WHERE UPPER(TRIM(t2.PASSTYPE)) IN ('TEST', 'INTERRUPT', 'TEST ISR', 'TEST INTERRUPT', 'RETESTBIN') AND UPPER(TRIM(t2.LAYERNAME)) <> 'ABANDONED'
    ORDER BY t2.TESTEND DESC NULLS LAST, t1.SLOT, t2.PASSID, t2.PASSNUM
    FETCH FIRST :lim ROWS ONLY
) topn
INNER JOIN INFLAYERBINLIST lb ON lb.ROWID = topn.lb_rid
INNER JOIN INFCONTROL ic ON ic.KEYNUMBER = lb.KEYNUMBER
ORDER BY lb.TESTEND DESC NULLS LAST, ic.SLOT, lb.PASSID, lb.PASSNUM
```

### 1.2 带筛选时 `WHERE` 形态

在 `WHERE t2.PASSTYPE = 'TEST'` 之后追加 ` AND ` + `parseInfcontrolLayerBinsV3Query` 生成的 **`whereAndSql`**（片段内已为 `AND` 连接的条件，**不含** `WHERE`）。

**示例**（`whereAndSql` 含 `TRIM(t1.DEVICE)` 与 `t2.TESTEND` 下界）：

```sql
SELECT
ic.DEVICE,
    ic.LOT,
    ic.SLOT,
    ic.MESLOT,
    lb.TESTERID,
    lb.TSTYPE,
    lb.CARDID,
    lb.PIBID,
    lb.PROBE,
    lb.GROSSDIE,
    lb.PASSID,
    lb.PASSNUM,
    lb.TESTSTART,
    lb.TESTEND,
    lb.LAYERNAME,
    lb.PASSRESUME,
    lb.PASSTYPE,
    lb.PASSBIN,
    lb.BIN1,
    lb.BIN2,
    lb.BIN3,
    lb.BIN4,
    lb.BIN5,
    lb.BIN6,
    lb.BIN7,
    lb.BIN8,
    lb.BIN9,
    lb.BIN10,
    lb.BIN11,
    lb.BIN12,
    lb.BIN13,
    lb.BIN14,
    lb.BIN15,
    lb.BIN16,
    lb.BIN17,
    lb.BIN18,
    lb.BIN19,
    lb.BIN20,
    lb.BIN21,
    lb.BIN22,
    lb.BIN23,
    lb.BIN24,
    lb.BIN25,
    lb.BIN26,
    lb.BIN27,
    lb.BIN28,
    lb.BIN29,
    lb.BIN30,
    lb.BIN31,
    lb.BIN32,
    lb.BIN33,
    lb.BIN34,
    lb.BIN35,
    lb.BIN36,
    lb.BIN37,
    lb.BIN38,
    lb.BIN39,
    lb.BIN40,
    lb.BIN41,
    lb.BIN42,
    lb.BIN43,
    lb.BIN44,
    lb.BIN45,
    lb.BIN46,
    lb.BIN47,
    lb.BIN48,
    lb.BIN49,
    lb.BIN50,
    lb.BIN51,
    lb.BIN52,
    lb.BIN53,
    lb.BIN54,
    lb.BIN55,
    lb.BIN56,
    lb.BIN57,
    lb.BIN58,
    lb.BIN59,
    lb.BIN60,
    lb.BIN61,
    lb.BIN62,
    lb.BIN63,
    lb.BIN64,
    lb.BIN65,
    lb.BIN66,
    lb.BIN67,
    lb.BIN68,
    lb.BIN69,
    lb.BIN70,
    lb.BIN71,
    lb.BIN72,
    lb.BIN73,
    lb.BIN74,
    lb.BIN75,
    lb.BIN76,
    lb.BIN77,
    lb.BIN78,
    lb.BIN79,
    lb.BIN80,
    lb.BIN81,
    lb.BIN82,
    lb.BIN83,
    lb.BIN84,
    lb.BIN85,
    lb.BIN86,
    lb.BIN87,
    lb.BIN88,
    lb.BIN89,
    lb.BIN90,
    lb.BIN91,
    lb.BIN92,
    lb.BIN93,
    lb.BIN94,
    lb.BIN95,
    lb.BIN96,
    lb.BIN97,
    lb.BIN98,
    lb.BIN99,
    lb.BIN100,
    lb.BIN101,
    lb.BIN102,
    lb.BIN103,
    lb.BIN104,
    lb.BIN105,
    lb.BIN106,
    lb.BIN107,
    lb.BIN108,
    lb.BIN109,
    lb.BIN110,
    lb.BIN111,
    lb.BIN112,
    lb.BIN113,
    lb.BIN114,
    lb.BIN115,
    lb.BIN116,
    lb.BIN117,
    lb.BIN118,
    lb.BIN119,
    lb.BIN120,
    lb.BIN121,
    lb.BIN122,
    lb.BIN123,
    lb.BIN124,
    lb.BIN125,
    lb.BIN126,
    lb.BIN127,
    lb.BIN128,
    lb.BIN129,
    lb.BIN130,
    lb.BIN131,
    lb.BIN132,
    lb.BIN133,
    lb.BIN134,
    lb.BIN135,
    lb.BIN136,
    lb.BIN137,
    lb.BIN138,
    lb.BIN139,
    lb.BIN140,
    lb.BIN141,
    lb.BIN142,
    lb.BIN143,
    lb.BIN144,
    lb.BIN145,
    lb.BIN146,
    lb.BIN147,
    lb.BIN148,
    lb.BIN149,
    lb.BIN150,
    lb.BIN151,
    lb.BIN152,
    lb.BIN153,
    lb.BIN154,
    lb.BIN155,
    lb.BIN156,
    lb.BIN157,
    lb.BIN158,
    lb.BIN159,
    lb.BIN160,
    lb.BIN161,
    lb.BIN162,
    lb.BIN163,
    lb.BIN164,
    lb.BIN165,
    lb.BIN166,
    lb.BIN167,
    lb.BIN168,
    lb.BIN169,
    lb.BIN170,
    lb.BIN171,
    lb.BIN172,
    lb.BIN173,
    lb.BIN174,
    lb.BIN175,
    lb.BIN176,
    lb.BIN177,
    lb.BIN178,
    lb.BIN179,
    lb.BIN180,
    lb.BIN181,
    lb.BIN182,
    lb.BIN183,
    lb.BIN184,
    lb.BIN185,
    lb.BIN186,
    lb.BIN187,
    lb.BIN188,
    lb.BIN189,
    lb.BIN190,
    lb.BIN191,
    lb.BIN192,
    lb.BIN193,
    lb.BIN194,
    lb.BIN195,
    lb.BIN196,
    lb.BIN197,
    lb.BIN198,
    lb.BIN199,
    lb.BIN200,
    lb.BIN201,
    lb.BIN202,
    lb.BIN203,
    lb.BIN204,
    lb.BIN205,
    lb.BIN206,
    lb.BIN207,
    lb.BIN208,
    lb.BIN209,
    lb.BIN210,
    lb.BIN211,
    lb.BIN212,
    lb.BIN213,
    lb.BIN214,
    lb.BIN215,
    lb.BIN216,
    lb.BIN217,
    lb.BIN218,
    lb.BIN219,
    lb.BIN220,
    lb.BIN221,
    lb.BIN222,
    lb.BIN223,
    lb.BIN224,
    lb.BIN225,
    lb.BIN226,
    lb.BIN227,
    lb.BIN228,
    lb.BIN229,
    lb.BIN230,
    lb.BIN231,
    lb.BIN232,
    lb.BIN233,
    lb.BIN234,
    lb.BIN235,
    lb.BIN236,
    lb.BIN237,
    lb.BIN238,
    lb.BIN239,
    lb.BIN240,
    lb.BIN241,
    lb.BIN242,
    lb.BIN243,
    lb.BIN244,
    lb.BIN245,
    lb.BIN246,
    lb.BIN247,
    lb.BIN248,
    lb.BIN249,
    lb.BIN250,
    lb.BIN251,
    lb.BIN252,
    lb.BIN253,
    lb.BIN254,
    lb.BIN255,
    lb.PASSRESULT
FROM (
    SELECT t2.ROWID AS lb_rid
    FROM INFCONTROL t1
    INNER JOIN INFLAYERBINLIST t2 ON t1.KEYNUMBER = t2.KEYNUMBER
WHERE UPPER(TRIM(t2.PASSTYPE)) IN ('TEST', 'INTERRUPT', 'TEST ISR', 'TEST INTERRUPT', 'RETESTBIN') AND UPPER(TRIM(t2.LAYERNAME)) <> 'ABANDONED' AND UPPER(TRIM(t1.DEVICE)) = UPPER(:ic3_device) AND t2.TESTEND >= :ic3_testend_lo
    ORDER BY t2.TESTEND DESC NULLS LAST, t1.SLOT, t2.PASSID, t2.PASSNUM
    FETCH FIRST :lim ROWS ONLY
) topn
INNER JOIN INFLAYERBINLIST lb ON lb.ROWID = topn.lb_rid
INNER JOIN INFCONTROL ic ON ic.KEYNUMBER = lb.KEYNUMBER
ORDER BY lb.TESTEND DESC NULLS LAST, ic.SLOT, lb.PASSID, lb.PASSNUM
```

---

## 2. `GET /api/v3/yield-monitor-triggers/v3`

### 2.1 仅固定 `TYPE`（`delta_diff`），无其它可选筛选

`parseYieldMonitorTriggerV3Query` 始终包含 **`UPPER(TRIM(t."TYPE")) = UPPER(:v3_type_scope)`**（`:v3_type_scope` 绑定为 `delta_diff`）；响应 `filters.typeScope` 回显。再与其余 AND 条件拼接。

```sql
SELECT t.*
FROM (
    SELECT t.ROWID AS rid
    FROM YMWEB_YIELDMONITORTRIGGER t
WHERE UPPER(t."TYPE") = :v3_type_scope AND NOT REGEXP_LIKE(t.LOTID, '^(kk|gg|c)', 'i') AND t.TIME_STAMP >= :v3_ts_lo AND t.TIME_STAMP <= :v3_ts_hi
ORDER BY t.TIME_STAMP DESC NULLS LAST
    FETCH FIRST :lim ROWS ONLY
) topn
INNER JOIN YMWEB_YIELDMONITORTRIGGER t ON t.ROWID = topn.rid
ORDER BY t.TIME_STAMP DESC NULLS LAST
```

### 2.2 另有设备 / 时间等筛选时

`parseYieldMonitorTriggerV3Query` 生成完整 `WHERE ...` 行（含 `WHERE` 关键字），插入在 `FROM` 与 `ORDER BY` 之间。

**示例**（`device` + `timeStampBegin`，且含固定 `TYPE`）：

```sql
SELECT t.*
FROM (
    SELECT t.ROWID AS rid
    FROM YMWEB_YIELDMONITORTRIGGER t
WHERE UPPER(t."TYPE") = :v3_type_scope AND NOT REGEXP_LIKE(t.LOTID, '^(kk|gg|c)', 'i') AND UPPER(t.DEVICE) = :v3_device AND t.TIME_STAMP >= :v3_ts_lo
ORDER BY t.TIME_STAMP DESC NULLS LAST
    FETCH FIRST :lim ROWS ONLY
) topn
INNER JOIN YMWEB_YIELDMONITORTRIGGER t ON t.ROWID = topn.rid
ORDER BY t.TIME_STAMP DESC NULLS LAST
```

---

## 3. 与仓库保持同步

修改 SQL 后执行 **`npm run docs:api-v3`**（内部先 `tsc` 再运行本脚本）可重新生成本文件。
