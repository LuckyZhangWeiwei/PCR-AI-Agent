...
11. **产量 v3 与 `TYPE`**：**`GET …/yield-monitor-triggers/v3`** 与 **`…/v3/aggregate`** **不支持** **`type`** 查询参数，也**不要**在 **`dimensions`** 里写 **`type`**；库列 **`TYPE`** 仍在列表每行 JSON 中返回，按异常类型统计请在客户端处理。
...
| **`…/yield-monitor-triggers/v3/aggregate`**（产量触发） | 这段时间里，**按机台、料号、探针卡、某一天……分桶**，**每个桶里报警 / 触发发生了多少次**？（**不按**库列 **`TYPE`**（异常类型）筛选或分组。） | **数条数**：满足条件的一行触发记录 = **COUNT 加 1** | **`dimensions`**（**必填**，1～5 个维度，逗号分隔；**不含** **`type`**）、**`groupTop`**（前几名，默认 25） | **不要**用列表的 **`limit`** 假装在做全量统计；没有 **`v3`** 的旧地址 **`…/yield-monitor-triggers/aggregate`** **没挂路由**，会 **404** |
...
**`/yield-monitor-triggers/v3`**：全列、时间降序、**`FETCH FIRST`**；**不支持** **`type`** 查询参数（**`TYPE`** 仍在每行 JSON 里，仅不能按该列筛选）。**`/yield-monitor-triggers/v3/aggregate`**：**必填 `dimensions`**（1～5 维；**不允许**含 **`type`**），**`timeDay`** 与 **`timeHour`** 勿同现。旧路径 **`GET …/yield-monitor-triggers/aggregate`**（无 **`v3`**）**未挂载**，返回 **404**；全量计数只用 **`…/v3/aggregate`**（manifest **`deprecatedEndpoints`** 有说明）。
...
**通俗理解**：先在「某段时间、某台机台 / 某批料号」里圈出所有触发记录，再按你选的 **`dimensions`** 做「**每个桶里有多少条**」——例如 **`device,hostname`** 看各料号在各机台上各触发多少次；**`timeDay`** 看**自然日**趋势；**`hostname`** 看哪台机触发最多。**v3 不提供**按库列 **`TYPE`**（异常类型）的 **`type`** 查询参数，也**不能**把 **`type`** 写进 **`dimensions`**（该列仍会在列表每行 JSON 里返回，需自行在客户端筛）。
...
**与 v3 列表相同的筛选（节选）**：**`hostname`**、**`device`**、**`lotId`**、**`wafer`**、**`probeCard`**、**`pass`**；**`timeStampBegin` / `timeStampEnd`**（或 **`timeStampFrom` / `timeStampTo`**）。全部 **AND**。**不支持** **`type`** 筛选（传入 **`type=…`** 会得到 **400** **`VALIDATION_ERROR`**）。
...
| --- | --- |
| `limit` | 默认 **200**，最大 **500**（参数名 **`Limit` / `LIMIT`** 等亦可） |
| `hostname`, `device`, `lotId`, `pass`, `wafer`, `probeCard` | 字符串列 **`UPPER(TRIM)`** 与入参比较；**`pass`** 仍为数值全等；**不支持** **`type`** 筛选 |
...
**说明**：与 **§7.7**（作用、通俗、传参示例）一致；**必填 `dimensions`**（**不得**含 **`type`**）；**`documentation`** 为固定中文说明。时间窗等筛选与 **§8.6** 相同。
